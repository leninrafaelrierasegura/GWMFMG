...
truncate_to_three <- function(x) {
  truncated <- floor(x * 1000) / 1000
  sprintf("%.3f", truncated)
}
```
...
save(tau_from_graphlme, kappa_from_graphlme, file = here::here("data_files/tau_from_graphlme.RData"))
```


```{r}
load("~/Desktop/new_graph_lme_results.RData")
summary(RES_EXP)
...
choose_index <- seq(2, nrow(mse_df), by = 3)[1:17]
...
label_mapping <- c(
  ...
  "Statnu1.5" = "2", 
  "Nonstatnu1.5" = "2", 
  "Statnuest" = paste0("$", truncate_to_three(alpha_stat), "\\;(\\mathtt{est})$"), 
  "Nonstatnuest" = paste0("$", truncate_to_three(alpha_nonstat), "\\;(\\mathtt{est})$")
)
