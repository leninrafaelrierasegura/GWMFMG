...
list_models_before_cv <- list(mean_and_mode_params_statnu0.5 = mean_and_mode_params_statnu0.5,
              ...
              GROUPS = GROUPS,
              GROUPS2 = GROUPS2)
save(list_models_before_cv, file = here::here("data_files/cv_models_TEST.RData"))
```


```{r, eval = TRUE}
load(here::here("data_files/cv_models_TEST.RData"))
```


# Estimated values

### Estimated parameters for the model with $\nu = 0.5$

```{r, eval = TRUE}
list_models_before_cv$mean_and_mode_params_statnu0.5
```

### Estimated parameters for the model with $\nu = 0.5$

```{r, eval = TRUE}
list_models_before_cv$mean_and_mode_params_statnu1.5
```

### Estimated parameters for the model with $\nu$ estimated

```{r, eval = TRUE}
list_models_before_cv$mean_and_mode_params_statnuest
```
...
p <- graph$plot(vertex_size = 0) +
  ...
  coord_fixed()
p <- plotly::ggplotly(p)
save(p, file = here::here("data_files/cv_groups_plot_TEST.RData"))
```
...
load(here::here("data_files/cv_groups_plot_TEST.RData"))
p
```
...
```
```{r}
AUXX0 <- list_models_before_cv$GROUPS2
...
```{r}
MG_cv_mse_statnu0.5 <- MG_cv_mse_statnu1.5 <- MG_cv_mse_statnuest <- rep(NA, length(distance))
for (j in 1:length(distance)) {
  print(j)
  slackr_msg(text = paste0("server 3: j =", j), channel = "#research")
  train_test_indices <- train_test_indices_from_GROUP2[[j]]
  MG_cv_mse_statnu0.5[j] <- as.numeric(MetricGraph::cross_validation(models = rspde_fit_statnu0.5, train_test_indexes = train_test_indices, true_CV = FALSE)$mse[1])
  MG_cv_mse_statnu1.5[j] <- as.numeric(MetricGraph::cross_validation(models = rspde_fit_statnu1.5, train_test_indexes = train_test_indices, true_CV = FALSE)$mse[1])
  MG_cv_mse_statnuest[j] <- as.numeric(MetricGraph::cross_validation(models = rspde_fit_statnuest, train_test_indexes = train_test_indices, true_CV = FALSE)$mse[1])
}

MG_list_to_save <- list(MG_cv_mse_statnu0.5 = MG_cv_mse_statnu0.5,
                        MG_cv_mse_statnu1.5 = MG_cv_mse_statnu1.5,
                        MG_cv_mse_statnuest = MG_cv_mse_statnuest)

save(MG_list_to_save, file = here::here("data_files/MG_list_to_save_TEST.RData"))
```
...
save(list_to_save, file = here::here("data_files/cv_result_TEST.RData"))
```


```{r}
load(here::here("data_files/cv_result_TEST.RData"))
...
combined_plot_TEST <- (mse_plot + ls_plot + 
...
myggsave(combined_plot_TEST, width = 9.22, height = 4.01)
```

```{r, eval = TRUE, out.width="922px", out.height="401px", fig.cap = captioner("MSE and negative Log-Score as functions of distance.")}
knitr::include_graphics(here::here("data_files/tikzpic/combined_plot_TEST.pdf"))
```
