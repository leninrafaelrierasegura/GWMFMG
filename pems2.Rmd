...
model_simple <- lm(y ~ mean_value, data = data)
summary(model_simple)

model_replicates <- lme4::lmer(y ~ mean_value + (1 | repl), data = data)
summary(model_replicates)


fit <- lm(y ~ mean_value + repl, data = data)
summary(fit)
```
...
mse_loocv_lm <- mean((data$y - pred_loocv)^2)
mse_loocv_lm

mse_loocv_lmer <- mean((data$y - pred_loocv_repl)^2)
mse_loocv_lmer
...
# -----------------------------------------------------------
alpha <- 0.5  # 0 = pure spatial, 1 = pure covariate
...
save(mse_loocv_lm, mse_loocv_lmer, best_KNN_mse, best_knn_mse, file = here("data_files/simple_linear_regression_results.RData"))
```

Below we show the MSE results for the simple linear regression `mse_loocv_lm` and the linear mixed effects model with replicates as random effects `mse_loocv_lmer`.


```{r, eval = TRUE}
load(here("data_files/simple_linear_regression_results.RData"))
cbind(mse_df[1,-1], data.frame(LM = mse_loocv_lm, 
                               LMrepl = mse_loocv_lmer,
                               kNNsimple = best_knn_mse,
