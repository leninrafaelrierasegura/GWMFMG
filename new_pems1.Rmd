...
save(Y_mean,Y_raw, PtE_raw, file = here::here("data_files/new_Y_mean.RData"))
save(Y_logstd, file = here::here("data_files/Y_logstd.RData"))
```
...
save(rspde_fit_stat_logstd, rspde_model_stat_logstd, file = here::here("data_files/new_rspde_fit_stat_and_model_logstd.RData"))
```


```{r, eval = TRUE}
load(here::here("data_files/new_rspde_fit_stat_and_model_logstd.RData"))
...
auxxx <- predict(rspde_fit_stat_logstd, newdata = rename(PtE, .edge_number = E, .distance_on_edge = length), ~Intercept + field)
tocompatewithlogstd <- auxxx$mean
save(tocompatewithlogstd, file = here::here("data_files/tocompatewithlogstd.RData"))
```


```{r, eval =TRUE, fig.width = 24, fig.height = 4, fig.cap = captioner("Predicted log std and observed log std")}
load(here::here("data_files/Y_logstd.RData"))
load(here::here("data_files/tocompatewithlogstd.RData"))
...
save(p12, p13, file = here::here("data_files/new_maps_zoom12and13from_stadia.RData"))
```


```{r}
load(here::here("data_files/new_maps_zoom12and13from_stadia.RData"))
```
...
ggsave(here("data_files/new_stand_log_sigma_3.png"), width = 11.2, height = 5.43, plot = final_plot, dpi = 500)
save(final_plot, file = here("data_files/new_stand_log_sigma_3.RData"))
```

```{r, eval = TRUE, out.width="1120px", out.height="543px", fig.cap = captioner("Standardized logarithm of the standard deviation of the first part of the data. Points represent the point value at each available location, and edges represent the $\\mathrm{std.cov}(s)$ covariate.")}
# Print the combined plot
knitr::include_graphics(here::here("data_files/new_stand_log_sigma_3.png"))
```
...
save(rspde_fit_stat_mean, rspde_model_stat_mean, file = here::here("data_files/new_rspde_fit_stat_and_model_mean.RData"))
```


```{r, eval = TRUE}
load(here::here("data_files/new_rspde_fit_stat_and_model_mean.RData"))
# Summarize the results
...
save(cov_for_mean_to_plot, cov_for_mean, file = here::here("data_files/new_cov_for_mean_to_plot_and_cov_for_mean.RData"))
```
...
load(here::here("data_files/new_Y_mean.RData"))
load(here::here("data_files/new_cov_for_mean_to_plot_and_cov_for_mean.RData"))
...
final_plot <- annotate_figure(combined_plot, left = textGrob("Latitude", rot = 90, vjust = 1, gp = gpar(cex = 0.8)),
                              bottom = textGrob("Longitude", vjust = -0.5, gp = gpar(cex = 0.8)))
ggsave(here("data_files/new_mean_better_3.png"), width = 11.2, height = 5.43, plot = final_plot, dpi = 500)
```


```{r, eval = TRUE, out.width="1120px", out.height="543px", fig.cap = captioner("Average speeds of the first part of the data. Points represent the point value at each available location, and edges represent the $\\mathrm{mean.cov}(s)$ covariate.")}
knitr::include_graphics(here::here("data_files/new_mean_better_3.png"))
```
...
final_plot <- annotate_figure(combined_plot, left = textGrob("Latitude", rot = 90, vjust = 1, gp = gpar(cex = 0.8)),
                              bottom = textGrob("Longitude", vjust = -0.5, gp = gpar(cex = 0.8)))
ggsave(here("data_files/new_replicate14_3.png"), width = 11.2, height = 5.43, plot = final_plot, dpi = 500)
```
...
knitr::include_graphics(here::here("data_files/new_replicate14_3.png"))
```


```{r}
save(graph, cov, cov_for_mean_to_plot, data_prd_list_mesh, file = here("data_files/new_pems_repl1_data.RData"))
```
